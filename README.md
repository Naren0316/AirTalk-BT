...
Two devices with Bluetooth radios pair directly and exchange messages over a Bluetooth serial (RFCOMM) socket. Every message is encrypted on the sender's device with AES-256-GCM and only decrypted on the recipient's device.
...
- Learning how offline transport and real end-to-end encryption are built from scratch
- A portfolio project that demonstrates both networking and applied cryptography
## Features
 
- **No SIM, Wi-Fi, or internet required** — works entirely over a direct Bluetooth (RFCOMM) connection
- **Real end-to-end encryption** — X25519 (ECDH) key exchange, HKDF-SHA256 key derivation, AES-256-GCM authenticated encryption; a fresh key pair every session
- **Tamper detection** — corrupted or modified messages are rejected, not silently accepted
- **Contact book** — save discovered devices by name, skip re-scanning every time
- **Automatic reconnection** — retries a failed connection before giving up with a clear error
- **Local, timestamped chat logs** — saved per conversation, never transmitted anywhere
- **23 automated tests** — the crypto handshake, encryption round-trip, tamper rejection, and contact/log storage are all covered, runnable with zero Bluetooth hardware
## How it works
...
Full protocol design — why RFCOMM over BLE, the message framing format, and the threat model — lives in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
To also run the test suite, additionally install `requirements-dev.txt` (see [Testing](#testing)).
...
A full recorded-demo script (what to run, in what order, on two devices) is in [`docs/DEMO.md`](docs/DEMO.md).
 
### Lower-level scripts
 
`discovery.py`, `server.py`, and `client.py` still work standalone — useful for understanding each layer individually, or quick manual testing without the contacts/menu system:
...
The encryption layer, contact book, and chat logging are covered by an automated test suite that runs without any Bluetooth hardware — the crypto tests use a local loopback socket pair (`socket.socketpair()`), which has the identical `send()`/`recv()` interface a real Bluetooth socket has.
...
Not covered by automated tests (needs real hardware — see [`docs/DEMO.md`](docs/DEMO.md) for a manual walkthrough): actual Bluetooth device discovery, RFCOMM connection/acceptance, and the interactive chat loop's I/O.
...
- **Chat logs are stored as plaintext locally** — readable to anyone with access to the device's filesystem, since they're decrypted for you to read. Never transmitted anywhere.
...
│   └── DEMO.md              # script for recording a demo
...
│   └── test_session.py
└── src/
    ├── discovery.py         # nearby device scanner
    ├── config.py            # shared service name/UUID
    ├── server.py            # RFCOMM server + encrypted chat (standalone)
    ├── client.py            # RFCOMM client + encrypted chat (standalone)
    ├── crypto_utils.py      # X25519 handshake, AES-256-GCM, framing
    ├── contacts.py          # local contact book (contacts.json)
    ├── session.py           # shared chat loop - timestamps + logging
    └── chat.py              # main menu-driven app (recommended entry point)
```
 
## Build roadmap
 
Built in 5 days, one layer at a time:
 
| Day | Milestone |
|-----|-----------|
| 1 | Repo setup, architecture design, Bluetooth device discovery |
| 2 | RFCOMM connection — server + client exchange plaintext messages |
| 3 | End-to-end encryption layer — X25519 key exchange + AES-256-GCM |
| 4 | Chat interface (CLI) — contacts, reconnection handling, local logging |
| 5 | Automated test suite, edge-case hardening, docs, demo script |
